...
namespace FFmpeg.SegmentAndMerge
{
    public sealed unsafe class H264VideoStreamEncoder : IDisposable
    {
        ...
        public H264VideoStreamEncoder(Stream stream, AVRational time_base, Size frameSize)
        {
            _stream = stream;
            ...
            _pCodecContext->time_base = time_base;
        ...
        public void Encode(AVFrame frame, AVFormatContext* output_format_context, int ticks)
        {
            ...
            try
            {
                ...
                var outputFormatStream = output_format_context->streams[pPacket->stream_index];
                pPacket->pts = frame.pts;
                pPacket->dts = frame.pts;
                //pPacket->pts = ticks*1000;
                //pPacket->dts = ticks * 1000;
                //pPacket->duration = 1 * 1000;

                //pPacket->duration = 1;
                Utils.ConfigureTimeStamps(pPacket, outputFormatStream, output_format_context);
                ffmpeg.av_interleaved_write_frame(output_format_context, pPacket);
        ...
        internal void CloseAndTrailingOutput(AVFormatContext* output_format_context)
        {
